...
import com.example.bakingapp.App;
...
public class JsonUtils {
    ...
    public static List<Baking> getBankings(String bak) {
        List<Baking> bakings = new ArrayList<>();
        try {
            JSONArray array = new JSONArray(bak);
            for (int i = 0; i < array.length(); i++) {
                JSONObject obj = array.getJSONObject(i);
                Baking baking = new Baking();
                baking.setId(obj.getInt("id"));
                baking.setName(obj.getString("name"));
                baking.setImage(obj.getString("image"));
                baking.setServings(obj.getInt("servings"));
                List<Baking.IngredientsBean> ingredientsBeans = new ArrayList<>();
                JSONArray arri = obj.getJSONArray("ingredients");
                for (int j = 0; j < arri.length(); j++) {
                    Baking.IngredientsBean ingredientsBean = new Baking.IngredientsBean();
                    JSONObject object = arri.getJSONObject(j);
                    ingredientsBean.setQuantity(object.getInt("quantity"));
                    ingredientsBean.setMeasure(object.getString("measure"));
                    ingredientsBean.setIngredient(object.getString("ingredient"));
                    ingredientsBeans.add(ingredientsBean);
                }
                JSONArray arrs = obj.getJSONArray("steps");
                baking.setIngredients(ingredientsBeans);
                List<Baking.StepsBean> stepsBeans = new ArrayList<>();
                for (int j = 0; j < arrs.length(); j++) {
                    Baking.StepsBean stepsBean = new Baking.StepsBean();
                    JSONObject object = arrs.getJSONObject(j);
                    stepsBean.setId(object.getInt("id"));
                    stepsBean.setShortDescription(object.getString("shortDescription"));
                    stepsBean.setDescription(object.getString("description"));
                    stepsBean.setVideoURL(object.getString("videoURL"));
                    stepsBean.setThumbnailURL(object.getString("thumbnailURL"));
                    stepsBeans.add(stepsBean);
                }
                baking.setSteps(stepsBeans);
                bakings.add(baking);
            }
            return bakings;
    ...
    public static List<Baking> getBankings(int id) {
        List<Baking> bakings = new ArrayList<>();
        try {
            JSONArray array = new JSONArray(App.getInstance().getResources().getString(id));
            for (int i = 0; i < array.length(); i++) {
                JSONObject obj = array.getJSONObject(i);
                Baking baking = new Baking();
                baking.setId(obj.getInt("id"));
